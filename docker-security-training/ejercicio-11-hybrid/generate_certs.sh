...
docker run --rm -v "$(pwd)/certs:/certs" openquantumsafe/oqs-ossl3 sh -c "
    cd /certs && \
    echo 'Generando CA Root (Hybrid Demo - ECDSA)...' && \
    openssl req -x509 -new -newkey ec -pkeyopt ec_paramgen_curve:prime256v1 -keyout ca_pqc.key -out ca_pqc.crt -nodes -subj '/CN=Hybrid PQC Lab Root CA' -days 365 && \
    echo 'Generando Clave y CSR del Servidor (Hybrid Demo - ECDSA)...' && \
    openssl req -new -newkey ec -pkeyopt ec_paramgen_curve:prime256v1 -keyout server.key -out server.csr -nodes -subj '/CN=localhost' && \
